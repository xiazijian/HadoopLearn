package cn.xwy.hadoop.secondarySort;
...
public class SecondarySort extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        String arg = "-D mapreduce.job.reduces=10 ./input/SecondarySort ./output/SecondarySort";
    ...
    public static class FirstPartitioner extends Partitioner<IntPair, IntWritable>
    {
        ...
        @Override
        public int getPartition(IntPair key, IntWritable value,int numReduceTasks)
        {
            //因为数据第一列代表月，第二列代表星期，所以得出indexRange这个值,按照月份升序星期降序全局排列
            int minIndex = 0;
            int maxIndex = 11 * 7 + 6;
            int indexRange = (maxIndex - minIndex) + 1;

            int indicesPerReducer = (int) Math.floor(indexRange / numReduceTasks);
            int index = (key.first - 1) * 7 + (7 - key.second);
